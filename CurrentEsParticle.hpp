...
struct CurrentEsParticle { // NOLINT(bugprone-exception-escape)
  int &identity() { return p.identity; }
  int const &identity() const { return p.identity; }

  bool operator==(CurrentEsParticle const &rhs) const {
...
private:
#ifdef EXCLUSIONS
  /** list of particles, with which this CurrentEsParticle has no non-bonded
   *  interactions
   */
  std::vector<int> el;
#endif
